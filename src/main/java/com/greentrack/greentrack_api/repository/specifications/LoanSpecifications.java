  ...
  public static Specification<LoanEntity> getLoans(LoanFilterDTO filter) {
    return (root, query, criteriaBuilder) -> {
      ...
      if (filter.startIssuedAt() != null) {
        predicates.add(
            criteriaBuilder.greaterThanOrEqualTo(
                root.get(FIELD_ISSUED_AT), filter.startIssuedAt()));
      }
      if (filter.endIssuedAt() != null) {
        predicates.add(
            criteriaBuilder.lessThanOrEqualTo(root.get(FIELD_ISSUED_AT), filter.endIssuedAt()));
      }
      ...
      if (filter.startReturnedAt() != null) {
        predicates.add(
            criteriaBuilder.greaterThanOrEqualTo(
                root.get(FIELD_RETURNED_AT), filter.startReturnedAt()));
      }
      if (filter.endReturnedAt() != null) {
        predicates.add(
            criteriaBuilder.lessThanOrEqualTo(root.get(FIELD_RETURNED_AT), filter.endReturnedAt()));
      }
      ...
      if (filter.employeeStatus() != null) {
        predicates.add(
            criteriaBuilder.equal(employeeJoin.get(FIELD_STATUS), filter.employeeStatus()));
      }
      ...
      if (StringUtils.hasText(filter.globalSearch())) {
        ...
        Predicate globalPredicate =
            criteriaBuilder.or(
                criteriaBuilder.like(
                    criteriaBuilder.lower(employeeJoin.get(FIELD_FULLNAME)), searchPattern),
                criteriaBuilder.like(
                    criteriaBuilder.lower(employeeJoin.get(FIELD_EMAIL)), searchPattern),
                criteriaBuilder.like(
                    criteriaBuilder.lower(deviceJoin.get(FIELD_NAME)), searchPattern),
                criteriaBuilder.like(
                    criteriaBuilder.lower(deviceJoin.get(FIELD_BRAND)), searchPattern));
