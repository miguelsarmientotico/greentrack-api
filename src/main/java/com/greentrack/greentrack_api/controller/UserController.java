...
public class UserController {
  ...
  public ResponseEntity<PagedResponse<UserResponseDTO>> getAllUsers(
    ...
    Page<UserEntity> usersPage = userService.searchUsersAdvanced(filter, pageable);

    List<UserResponseDTO> userDTOs =
        usersPage.getContent().stream().map(mapper::entityToApiResponse).toList();

    PagedResponse<UserResponseDTO> response =
        new PagedResponse<>(
            userDTOs,
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "404",
            description = "${api.responseCodes.notFound.description:Not Configured}")
      })
  @PreAuthorize("hasRole('ADMIN')")
  @GetMapping("/{userId}")
  public ResponseEntity<UserResponseDTO> getUserById(@PathVariable UUID userId) {
    LOG.debug("Admin consultando perfil de usuario ID: {}", userId);
  ...
  public ResponseEntity<UserResponseDTO> createUser(
      @Validated(OnCreate.class) @RequestBody UserDTO userDTO, Authentication authentication) {
    LOG.info(
        "👤 Admin '{}' creando NUEVO USUARIO: {} (Email: {})",
  ...
  public ResponseEntity<UserResponseDTO> updateUser(
    ...
    LOG.info(
        "🛠️ Admin '{}' modificando perfil de usuario ID: {}", authentication.getName(), userId);
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "204",
            description = "${api.responseCodes.noContent.description:Not Configured}"),
        @ApiResponse(
            responseCode = "404",
            description = "${api.responseCodes.notFound.description:Not Configured}")
      })
  ...
  public ResponseEntity<Void> deleteUser(@PathVariable UUID userId, Authentication authentication) {
    LOG.warn(
        "🗑️ Admin '{}' ELIMINANDO permanentemente al usuario ID: {}",
        authentication.getName(),
        userId);
  ...
  public ResponseEntity<Void> deleteUsersBulk(
      @RequestBody Map<String, List<UUID>> payload, Authentication authentication) {
    List<UUID> ids = payload.get("ids");
    int count = (ids != null) ? ids.size() : 0;

    LOG.warn(
        "🗑️⚠️ Admin '{}' ejecutó ELIMINACIÓN MASIVA de {} usuarios.",
        authentication.getName(),
        count);
