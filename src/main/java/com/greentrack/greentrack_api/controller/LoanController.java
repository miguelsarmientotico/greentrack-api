...
public class LoanController {
  ...
  public ResponseEntity<PagedResponse<LoanResponseDTO>> getAllLoans(
    ...
    PagedResponse<LoanResponseDTO> response =
        new PagedResponse<>(
            dtoPage.getContent(),
            dtoPage.getNumber() + 1,
            dtoPage.getSize(),
  ...
  public ResponseEntity<LoanResponseDTO> createLoan(
      @Validated(OnCreate.class) @RequestBody LoanDTO loanDTO, Authentication authentication) {
    LOG.info(
  ...
  public ResponseEntity<LoanResponseDTO> returnLoan(
      @PathVariable UUID loanId, Authentication authentication) {
    LOG.info(
        "🔄 Usuario Admin '{}' procesando DEVOLUCIÓN de préstamo ID: {}",
        authentication.getName(),
        loanId);
  ...
  public ResponseEntity<Void> deleteLoan(@PathVariable UUID loanId, Authentication authentication) {
    LOG.warn(
        "⚠️ Eliminación de registro de préstamo ID: {} solicitada por '{}'",
        loanId,
        authentication.getName());
  ...
  public ResponseEntity<Void> deleteLoansBulk(
    ...
    LOG.warn(
        "⚠️ ELIMINACIÓN MASIVA de {} préstamos iniciada por '{}'", count, authentication.getName());
