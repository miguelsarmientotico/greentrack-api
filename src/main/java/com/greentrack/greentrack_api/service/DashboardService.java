...
public class DashboardService {
  ...
  public DashboardResponseDTO getDashboard() {
    ...
    long borrowedDevices =
        allDevices.stream()
            .filter(device -> DeviceStatusEnum.PRESTADO.equals(device.getStatus()))
            .count();

    long activeLoans =
        allLoans.stream().filter(loan -> LoanStatusEnum.ACTIVO.equals(loan.getStatus())).count();

    long returnedLoans =
        allLoans.stream().filter(loan -> LoanStatusEnum.DEVUELTO.equals(loan.getStatus())).count();
    ...
    return DashboardResponseDTO.builder()
        .users(DashboardResponseDTO.UserStats.builder().total(totalUsers).build())
        .devices(
            DashboardResponseDTO.DeviceStats.builder()
                .total((long) allDevices.size())
                .available(availableDevices)
                .borrowed(borrowedDevices)
                .build())
        .loans(
            DashboardResponseDTO.LoanStats.builder()
